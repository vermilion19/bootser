...
### 만들면서 더한 규칙 — **다시 만들지 않고 빠진 것만 채운다**

꼬리를 늘릴 때 투영을 통째로 다시 만들면 안 된다. 행에 `notified_at` 이 있어서,
지우고 다시 넣으면 **이미 보낸 알림이 「안 보낸 것」으로 되살아나** 다음 회차에 또
나간다 — 생일 알림을 두 번 받는 사람이 생기고 **그것을 우리가 알 방법이 없다.**

고쳐 쓰기(`save`)도 안 된다. 같은 키로 만든 새 객체는 `notifiedAt` 이 비어 있어
**병합하는 순간 그 값이 지워진다.** 그래서 있는 것을 먼저 읽고 없는 것만 넣는다.

> 수정(`edit`)이 통째로 다시 만드는 것은 괜찮다 — 거기서는 **사람이 값을 바꾼 것**이라
> 옛 알림 기록이 더 이상 맞지 않는다.

**알림 날짜가 이미 지난 줄은 만들지 않는다.** 오늘이 생일인 사람이 「7일 전에 알려
줘」로 등록하면 그 줄의 알림 날짜는 일주일 전이다. 만들어 두면 태어나자마자
「놓친 알림」으로 세어지고, 그 수는 원래 **「스케줄러가 멈춰 있었다」를 뜻해야 하는
신호**다.
...
`source_count` 와 `stored_count` 를 **둘 다** 둔다. ~~차이가 곧 A-10 이 거른 비-`Public`
건수이고~~ — **이 한 줄이 틀렸다 (2026-09-15, 착수 5 에서 드러남).**

> **A-10 은 비-`Public` 을 거르지 않는다.** 노출만 막고 저장은 한다 (SPEC §9.3 ·
> "버리면 축을 열 때 전체 재동기화가 필요하다"). 그러므로 그 차이는 **설계상 0 이다.**
>
> 그래도 두 칼럼은 그대로 둔다. 실제로 잡는 것이 다를 뿐 쓸모는 있다 —
> **원천이 같은 자연키를 두 번 준 경우**(뒤엣것을 버린다)와 **값이 이상해 행을 만들지
> 못한 경우**다. 둘 다 평소 0 이므로 **0이 아닌 것 자체가 신호**라는 결론은 안 바뀐다.
...
| **체크섬 검증** (적용된 것과 파일이 같은가) | 없다 (§14.5 가 왜 안 두는지 적는다) |
| ~~**변경 이력** (2차 스키마 변경)~~ | **되찾았다** — `schema_change` 표 (§14.3) |
| ~~**적용 순서 강제**~~ | **되찾았다** — 리비전 가드 (§14.3) |
...
**`schema.sql` 은 빈 DB 를 세우는 스크립트이지 변경 이력이 아니다.** 이미 자료가 찬 DB 를
고치는 절차는 **§14 가 정한다** (S-6 — 2026-09-15 에 닫혔다). 그리고 §14 R1 이 위 문장을
규칙으로 바꾼다 — *`schema.sql` 은 언제나 「지금의 최종 모양」이어야 하므로, 변경 스크립트를
쓰는 사람이 이 파일도 같이 고친다.*
...
| ~~**S-6**~~ | ~~**2차 스키마 변경을 어떻게 굴리나**~~ | **닫힘 (2026-09-15) → §14.** 번호 붙인 변경 스크립트 · DB 안의 이력표(`schema_change`) · 리비전 가드. 착수 3 이 `country` 에 행을 넣으면 「빈 DB」가 끝나므로 그 앞에서 닫았다 | 끝남 |
...
# 14. 2차 스키마 변경 — 자료가 찬 DB 를 고치는 절차 (S-6)

> **닫은 날 2026-09-15.** 착수 3 이 `country` 에 204행을 넣는 순간 「빈 DB」가 끝나므로
> **그 앞에서** 닫는다. 자료가 찬 뒤에 절차를 정하면, 절차가 없던 동안 무엇이 적용됐는지를
> 되짚을 방법이 없다.

## 14.1 무엇이 비어 있었나

§11.3 이 정한 것은 **빈 DB 를 세우는 절차**다. `psql -f schema.sql` 한 줄이고 그것으로 끝난다.

자료가 찬 뒤에는 그 한 줄을 다시 쓸 수 없다. `CREATE TABLE` 이 이미 있는 표를 만나 터지고,
터지지 않게 고치면 §11.2 가 버린 「나」가 된다.

§11.4 가 도구 없이 잃는 것 넷을 적어 두었다. **이 절이 그중 둘을 되찾는다.**

| 잃은 것 | 이 절이 되찾나 |
| --- | --- |
| 적용을 강제하는 힘 | **아니오** — 사람이 돌려야 한다 |
| 체크섬 검증 | **아니오** (§14.5 가 까닭을 적는다) |
| **변경 이력** | **예** — `schema_change` |
| **적용 순서 강제** | **예** — 리비전 가드 |
...
| | 방법 | 평가 |
| --- | --- | --- |
| 가 | 절차 없음. 그때그때 `psql` 로 `ALTER` | **버림.** 무엇이 적용됐는지 아무도 모른다. 인스턴스가 둘이면 한쪽만 고쳐진 채로 도는데, `validate` 는 **부팅하는 쪽에서만** 터지므로 이미 떠 있는 인스턴스는 계속 돈다 |
| 나 | `schema.sql` 을 멱등하게(`IF NOT EXISTS`) 만들어 매번 다시 돌린다 | **버림.** 칼럼의 타입을 바꾸는 것도, 자료를 옮기는 것도, 무엇을 지우는 것도 `IF NOT EXISTS` 로는 쓸 수 없다. 표현할 수 있는 변경이 「더하기」뿐이다 |
| 다 | 결정을 뒤집고 Flyway 를 넣는다 | **지금은 갈래가 아니다** (§11.2 에서 닫혔다). 다시 열 방아쇠는 §14.6 |
| **라** | **번호 붙인 변경 스크립트 + DB 안의 이력표 + 리비전 가드** | **채택** |

**「라」는 마이그레이션 도구의 최소 재구현이다.** 그것을 숨기지 않는다 — 숨기면 재구현이
커지는 것을 아무도 못 알아본다. 그래서 §14.6 에 **언제 이 결정을 다시 여는지**를 함께 적는다.

## 14.3 모양

```
scripts/
├── schema.sql              ← 빈 DB 용. 언제나 「지금의 최종 모양」이다
└── changes/
    ├── README.md           ← 이 절의 요약. 변경을 쓰는 사람이 여기를 먼저 본다
    └── 0002-{무엇을}.sql    ← 자료가 찬 DB 용. 리비전 1 → 2
```

번호는 **이름이 아니라 리비전**이다. `schema.sql` 이 리비전 1 을 세우므로 첫 변경이 `0002` 다.

변경 스크립트는 **가드로 시작해 이력으로 끝난다.**
...
**가드가 이 설계에서 제일 값싼 부품이고 제일 많이 건진다.** 도구가 주는 것 중 우리에게 실제로
아쉬운 것은 체크섬이 아니라 **순서**다 — `0003` 을 `0002` 보다 먼저 돌리면 대개 그냥 돌아가고,
**틀린 채로 돌아간다.** 이미 적용한 것을 한 번 더 돌리는 것도 가드가 막는다.

적용은 §11.3 과 같은 모양이다.

```bash
psql "$SPRING_DATASOURCE_URL" -v ON_ERROR_STOP=1 -1 -f scripts/changes/0002-add-holiday-name-ko.sql
```

`-1` 을 더한 것이 `schema.sql` 과 다른 점이다. **가드와 변경과 이력이 한 트랜잭션에 있어야**
중간에 터졌을 때 이력만 남는 일이 없다. 예외는 R4 하나다.

## 14.4 규칙 다섯

| | 규칙 | 안 지키면 |
| --- | --- | --- |
| **R1** | **한 변경은 두 파일을 같이 고친다** — `changes/NNNN-*.sql` 과 `schema.sql`(그리고 §12) | 새로 세운 DB 와 고친 DB 가 갈라진다. **둘 다 뜨고, 둘 다 틀려 보이지 않는다** |
| R2 | 변경 스크립트는 **가드로 시작하고 이력 `INSERT` 로 끝난다** | 순서가 안 지켜지고, 무엇이 적용됐는지 모른다 |
| R3 | **되돌리기(down) 스크립트를 쓰지 않는다.** 잘못 갔으면 **앞으로 가는 변경을 하나 더** 쓴다 | 되돌릴 수 있다고 믿게 된다. **자료를 지운 변경은 되돌아오지 않는데**, down 스크립트가 있으면 그 사실이 가려진다 |
| R4 | 자료가 찬 표에 인덱스를 더할 때는 `CREATE INDEX CONCURRENTLY` (§11.5). 그 문장은 **트랜잭션 블록 안에서 못 돈다** — 그 스크립트만 `-1` 없이 돌리고, 파일 머리에 그렇게 적는다 | 인덱스가 서는 내내 그 표의 쓰기가 막힌다 |
| R5 | 자료 이행(DML)이 든 변경은 **되돌릴 수 없는 것으로 다룬다.** 먼저 `pg_dump` | — |

**R1 이 이 절의 전부다.** §11.5 는 *"`schema.sql` 은 변경 이력이 아니라 지금의 최종 모양"*
이라고 적었는데, 그 문장은 자료가 차기 전에는 저절로 참이었다. 이제는 **고칠 때마다 손으로
지켜야 하는 규칙**이 된다. 그래서 §14.5 가 그것을 무엇이 보는지 적는다.

## 14.5 지켜지는지 무엇이 보나

| 무엇을 보나 | 누가 | 지금 도나 |
| --- | --- | --- |
| 리비전이 이어지는가 · `schema.sql` 의 리비전이 마지막 변경과 같은가 · 가드와 이력이 있는가 | `SchemaScriptTest` | **돈다 (Docker 불필요)** |
| 표 · 인덱스 · `CHECK` 가 다 있는가 | `SchemaIndexTest` | Docker 있는 자리에서 |
| 엔티티와 스키마가 어긋나지 않았는가 | 운영 부팅 시 `ddl-auto: validate` | 뜬다 |
| **`schema.sql` 로 세운 DB 와 (기준 + 변경들) 로 세운 DB 가 같은가** | 동등성 테스트 | **아직 없다** → §14.7 |

**체크섬은 두지 않는다.** 사람이 파일 해시를 손으로 넣어야 하고, *사람이 넣는 해시는 사람이
안 넣으면 없는 것과 같다.* 대신 위의 넷이 **구조**를 본다 — 적용된 결과가 파일과 같은 모양인지를
보는 것이 체크섬이 하려던 일의 본체다.

**다만 되찾지 못한 것 하나를 적어 둔다.** 자료 이행(DML)이 실제로 파일대로 돌았는지는
**아무도 안 본다.** 구조는 남지만 옮겨진 값은 구조에 안 보인다. 그것이 §14.6 의 방아쇠 2번이
DML 을 콕 집는 까닭이다.

## 14.6 도구를 다시 논의하는 방아쇠

§9.3(파티셔닝)과 같은 모양이다. **언제 다시 열 것인지를 지금 적어 둔다.**

셋 중 하나라도 걸리면 §11.2 를 다시 연다.

1. 변경 스크립트가 **다섯을 넘을 때**
2. **자료 이행(DML)이 든 변경이 처음 생길 때** — §14.5 가 못 보는 자리다
3. 적용 대상 환경이 **셋을 넘을 때** (지금은 `home` · `docker` 둘)

**재구현이 커지는 순간 「도구를 안 쓴다」의 이득이 사라진다.** 도구를 안 쓰기로 한 것은
도구가 나빠서가 아니라 **이 서비스의 변경이 드물 것이라고 봤기 때문**이고, 위 셋은 전부
그 전제가 틀렸다는 신호다.

## 14.7 첫 변경이 데려와야 하는 것 — 동등성 테스트

지금 `changes/` 는 비어 있고, **비어 있는 동안은 R1 이 깨질 수가 없다.**
첫 변경을 쓰는 사람이 넷을 함께 한다.

1. 지금의 `schema.sql` 을 `scripts/baseline/r1.sql` 로 **얼린다**
2. `changes/0002-*.sql` 을 쓴다
3. `schema.sql` 과 §12 를 고쳐 **최종 모양**으로 만든다
4. **동등성 테스트**를 세운다 — 컨테이너 둘에 각각 `schema.sql` 과 `(r1.sql + changes/*)` 를
   넣고 `pg_catalog` 를 견준다. 다르면 R1 이 깨진 것이다

**4번을 지금 안 쓰는 까닭은 기준 파일이 없어서다.** 오늘 만들면 `r1.sql` 이 `schema.sql` 의
복사본이 되고, 갈라질 자리가 하나 더 생긴다 — **막으려던 고장을 먼저 만드는 셈**이다.
대신 그때까지는 §14.5 의 첫 줄(`SchemaScriptTest`)이 리비전이 이어지는지를 본다.

## 14.8 재기준 (re-baseline)

변경이 쌓이면 `(기준 + 변경 N개)` 사슬이 길어진다. **모든 환경이 리비전 R 이상일 때** 사슬을
끊을 수 있다.

1. `changes/` 의 것들을 `changes/archive/` 로 옮긴다
2. `schema.sql` 이 새 기준이 된다 (이미 최종 모양이므로 고칠 것이 없다 — R1 덕이다)
3. `scripts/baseline/r{R}.sql` 을 지금의 `schema.sql` 로 갈아 끼운다

**"모든 환경이 R 이상"을 확인할 방법은 `SELECT max(revision) FROM schema_change` 하나뿐이고,
확인하는 것은 사람이다.** 환경을 빠뜨리면 그 환경은 영영 못 따라온다 — 기준이 이미 지나갔기
때문이다. §14.6 의 방아쇠 3번이 환경 수를 세는 까닭이 이것이다.
