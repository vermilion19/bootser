...
**따라오는 이사 하나.** `checkpoints.json` 이 지금 `d-day-service/src/test/resources/astro/` 에 있다.
...
절기 한 해 = 뉴턴법 반복 24회, 회당 VSOP87 절단 급수 평가 몇 번이다. **수 ms 로 예상하나
측정하지 않았다.** 그러므로 이것을 **착수 조건**으로 건다.

> `SolarTermSolver.termsOf(year)` 의 p99 를 `astro-core` 구현 직후 JMH 로 잰다.
> **20ms 이하면 「다」를 확정한다.** 넘으면 콜드에도 짧은 TTL(10분)을 주되 **별도 Redis
> 논리 DB 또는 별도 인스턴스**에 둔다 — 그래야 핫과 메모리를 안 다툰다. (→ 열린 결정 §11-1)
...
| `dday-occurrence-rollforward` | 연 1회 (§2.3) |
...
| | 무엇 | 왜 이 자리 |
| --- | --- | --- |
| 0 | 게이트웨이 **4번** (§7.1) | 경로가 열리기 전에 막는다 |
| 1 | 모듈 골격 — `astro-core` 신설 · 루트 패키지 이동 · `core-observability`/ShedLock 의존 · 픽스처 이사 | **파일이 하나뿐인 지금이 제일 싸다** |
...
| 4 | `astro-core` 절기 · 삭망 + **2층 검산점 테스트** | 검산점이 이미 있다. **값을 못 검산한 채로 쌓지 않는다** |
...
| 7 | **ΔT 모델** (§6.5) | 1583~2999 전 구간에서 1분 허용오차를 지키는 모델 | 착수 4 |
...
| 12 | **소프트 삭제 보존 기간** | 원천에서 사라진 공휴일을 언제까지 들고 있나. 스키마와 함께 | `db-tuner` |
