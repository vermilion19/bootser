    ...
    class Solution {
        public int solution(String[] friends, String[] gifts) {
            ...
            for (String gift : gifts) {
                String[] strings = splitGiverAndTaker(gift);
                String giver = strings[0];
                String taker = strings[1];

                int giverIndex = indexMap.get(giver);
                int takerIndex = indexMap.get(taker);
                arr[giverIndex][takerIndex] += 1;
            ...
            return Arrays.stream(expect).max().orElse(0);
            }

        String[] splitGiverAndTaker(String gift) {
            String[] split = gift.split(" ");
            String giver = split[0];
            String taker = split[1];
            return new String[]{giver, taker};
        }
    }
}
