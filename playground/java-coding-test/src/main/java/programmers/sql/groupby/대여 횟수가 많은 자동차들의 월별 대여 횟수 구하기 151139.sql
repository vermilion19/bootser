문제 설명
다음은 어느 자동차 대여 회사의 자동차 대여 기록 정보를 담은 CAR_RENTAL_COMPANY_RENTAL_HISTORY 테이블입니다. CAR_RENTAL_COMPANY_RENTAL_HISTORY 테이블은 아래와 같은 구조로 되어있으며,
...
WHERE
  ...
  AND CAR_ID IN (
    -- [Step 1] 8월~10월 사이 총 대여 횟수가 5회 이상인 CAR_ID 찾기
    SELECT CAR_ID
