...
크리처는 생성일 기준으로 나이(일수)가 증가하고, 돌봄 행동과 이벤트(질병/부상, 배틀 결과)에 따라 상태가 변화한다. 크리처를 처음 분양받을 때는 1단계(STAGE_1)로만 생성 가능하며, 훈련장에서 노력치를 쌓고 조건을 충족하면 다음 단계로 진화한다. 두 크리처를 합체해 새로운 크리처를 생성할 수도 있다.
...
| 상태 | 의미 | 권장 범위(예) | 비고 |
| --- | --- | --- | --- |
| 나이(ageDays) | 생성일(createdAt) 기준 경과 일수 | 0~무제한 | 하루마다 +1, 생성 직후 0 |
| 성장 레벨(level) | 현재 성장 단계 | 1~5 | 분양 시 1단계 고정, 추후 확장 가능 |
...
| 노력치(effortPoints) | 훈련을 통해 축적하는 성장 포인트 | 0~16 | 4칸 × 4회 훈련, 진화 시 리셋 |
...
- **수면 중에는 먹이/치료/훈련/진화/활성전환 등의 행동이 차단된다.** 먼저 깨워야 행동 가능.
...
- 크리처는 5단계 성장 레벨(STAGE_1 ~ STAGE_5)을 가진다.
- 처음 분양받을 때는 반드시 1단계(STAGE_1)로만 생성 가능하다.
- 진화는 반드시 현재 단계의 다음 단계로만 가능하다(1→2→3→4→5 순차 진화).
- 성장 레벨은 추후 확장 가능한 구조로 설계한다.

### 6.2 진화

- 나이, 컨디션, 승률, 노력치 등의 조건을 충족하면 다음 단계로 진화할 수 있다.
- 진화 조건은 서버 설정값으로 관리하여 밸런싱을 쉽게 한다(예: ageDays>=A AND condition>=B AND winrate>=C AND effortSlots>=D).
- **진화 시 노력치(effortPoints)는 0으로 초기화된다.** 다음 단계에서 다시 훈련을 쌓아야 한다.

### 6.3 훈련(노력치)

- 훈련장에서 훈련하면 노력치(effortPoints)가 1씩 증가한다.
- 노력치는 총 4칸으로 구성되며, 훈련 4회마다 1칸이 채워진다(최대 16포인트 = 4칸).
- 수면 중에는 훈련할 수 없다(깨운 후 훈련 가능).
- 노력치가 최대(16)에 도달하면 더 이상 훈련할 수 없다.
- 진화 시 노력치는 0으로 리셋된다.

| 훈련 횟수 | effortPoints | 채워진 칸(effortSlots) |
| --- | --- | --- |
| 0회 | 0 | 0/4 |
| 4회 | 4 | 1/4 |
| 8회 | 8 | 2/4 |
...
| Creature | id, user_id, name, species, stage(STAGE_1~5), is_active, created_at | 크리처 기본 정보 (level=stage.level, ageDays=now-createdAt) |
| CreatureState | creature_id, age, health, hunger, condition, winrate, effort_points, updated_at | 상태 스냅샷(노력치 포함, 마지막 갱신 시각 중요) |
...
| POST | /creatures/{id}/actions/train | 훈련(노력치 +1, 수면 중 불가) |
| POST | /creatures/{id}/actions/evolve | 진화(다음 단계로, 노력치 리셋) |
...
- 상태 계산(오프라인 포함) + 기본 돌봄(먹이/취침/치료) + 수면 중 행동 차단
- 5단계 성장 레벨 + 훈련(노력치 4칸) + 진화(노력치 리셋)
- 질병/부상 최소 1~2종
